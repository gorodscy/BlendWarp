...
// force the interface into vector-drawing mode
void ImDraw::set_morphing_mode(void)
{
	// change the vector mode
	view_mode = Morph;
}

// pass the morphing datastructure
void ImDraw::set_morph(morphing* morph)
{
	morph_data = morph;
}

bool ImDraw::morphing_mode()
{
	return (view_mode == Morph);
}
	  ...
	  switch (event) {
		...
		case FL_PUSH: {
			...
			if (gl2vxl(mousex, mousey, i, j)) {
				...
				// drawing an expanding vector	
				if (Fl::event_button() == 1) {
					pos2_j = pos1_j = j;
					pos2_i = pos1_i = i;
					// draw an unfilled, non-persistent vector
					draw_object(new imdraw_vec(pos1_j, pos1_i, 
								    pos2_j, pos2_i,
								    2.0, 3.0, 1.0, 0.0, 0.0,
								    false));
					...
					// if button 3 is pressed, this should cause
					// editing of the closest drawn vector
					// we only do this if the morphing data structure
					// is initialized
					if (morph_data) {
						double P_i, P_j, Q_i, Q_j;
						if (morph_data->find_closest_line(i, j, isP, lineid, this)) 
							morph_data->modify_line(lineid, isP, i, j, this);
					}
				}
				expand = true;
		...
		case FL_RELEASE: {
			...
			if ((Fl::event_button() == 1) && (expand)) {
				// and add a persistent, filled vector to the display list
				//draw_object(new imdraw_vec(pos1_j, pos1_i, pos2_j, pos2_i, 
				//	                  2.0, 3.0, 1.0, 1.0, 0.0, true));
				if (morph_data)
					morph_data->add_line(pos1_i, pos1_j, pos2_i, pos2_j, this);
				// we stop changing the vector
				expand = false;
			} if ((Fl::event_button() == 3) && (expand)) 
				expand = false;
		}
		break;
	    default:
